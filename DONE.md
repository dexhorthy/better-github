...
- Collapsed the duplicated repository/actions/workflow route table into `src/repo-routes.ts`:
  - Added `registerRepositoryRoutes(app, deps)` for the shared `/api/repos`, `/api/repos/:owner/:repo`, Actions runs, cancel, workflow file CRUD, and signed push webhook routes.
  - `src/server.ts` now injects the Postgres workflow repository, local WebSocket broadcaster, local Freestyle helpers, and Bun auth middleware into the shared registrar.
  - `src/worker.ts` now injects the D1 workflow repository, Durable Object broadcaster, Worker secret/env bridge, injectable workflow-file fetcher, and `waitUntil` execution lifecycle into the same route registrar.
  - Preserved Worker behavior by keeping workflow execution fire-and-forget after queue insertion; the route returns `201 { id, status: "queued" }` without waiting for Freestyle VM execution.
  - Verified with `bun run typecheck`, `bun run lint`, `bun run test` (112 unit tests), `bun run test:e2e` (4 Playwright tests), and `agent-browser` smoke coverage of repo code view, Actions run list, and workflow editor.
