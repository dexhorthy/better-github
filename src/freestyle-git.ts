...
type FreestyleRepoEntry = {
	...
	branches?: Record<
		string,
		{ default: boolean; name: string; target?: string | null }
	>;
...
export async function findFreestyleRepoId(
	repoName: string,
): Promise<string | null> {
...
export async function fetchFreestyleRepoData(
	...
	try {
		...
		const [
			...
			path === ""
				? repo.contents.get({ path: "README.md" })
		...
		if (contentsResult.status === "fulfilled") {
			...
			if (contents.type === "dir") {
				const latestMessage =
					commits[0]?.message ?? "Synced from Freestyle Git";
		...
		if (
			...
			readmeResult.value.type === "file"
		) {
			readme = {
				text: Buffer.from(readmeResult.value.content, "base64").toString(
					"utf8",
				),
			};
		}
