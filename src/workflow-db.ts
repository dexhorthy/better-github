...
export async function insertWorkflowRun(run: WorkflowRun): Promise<void> {
	...
	await sql`
    INSERT INTO workflow_runs (id, workflow_name, repo_owner, repo_name, branch, commit_sha, status, conclusion, started_at, completed_at, logs)
    VALUES (${run.id}, ${run.workflowName}, ${run.repoOwner}, ${run.repoName}, ${run.branch}, ${run.commitSha}, ${run.status}, ${run.conclusion ?? null}, ${run.startedAt}, ${run.completedAt ?? null}, ${run.logs ?? null})
  `;
}
...
export async function updateWorkflowRun(
	id: string,
	updates: Partial<
		Pick<WorkflowRun, "status" | "conclusion" | "completedAt" | "logs">
	>,
): Promise<void> {
	await ensureWorkflowRunsTable();
	await sql`
