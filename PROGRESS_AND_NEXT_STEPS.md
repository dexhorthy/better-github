...
- Continued the `App.tsx` page split by extracting `RepoList` into `src/repo-list.tsx` (with focused tests in `src/repo-list.test.tsx`) and lifting the shared `timeAgo` helper into `src/format-time.ts`.
- 115 unit tests, 4 Playwright e2e tests, and full typecheck + Biome lint all passing.

## Highest priority task
<guidance> keep this a low level, the smallest individually testable unit </guidance>

Task: Continue the `App.tsx` page split by extracting the repo browser page (`RepoBrowser`) into a small page module with its existing behavior covered by focused tests. Lift the `RepoTab` type and the inline `path`/popstate URL effect along with it so the new module owns its own URL-sync state.
...
5. **Break up `App.tsx` (now ~1530 lines) into per-page modules.** *(in progress — `RepoList`, `RepoBreadcrumb`/`RepoHomeLink`, `ReadmePreview`/`LineNumberedCode` extracted)*
   - Files: `src/App.tsx`.
   - Problem: still declares several inline subcomponents (`AuthForm`, `WorkflowEditor`, `RunDetail`, `ActionsTab`, `RepoBrowser`) plus status-icon helpers, and runs ~4 state machines (auth, route, repo overview, runs) cascading via prop callbacks. Subcomponents can't be tested in isolation — `App.test.tsx` reaches most of them through the megacomponent surface.
