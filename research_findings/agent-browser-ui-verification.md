...
- For `src/repo-file-viewers.tsx` extraction verification, browser-smoked `http://127.0.0.1:5173/dexhorthy/better-github` and `http://127.0.0.1:5173/dexhorthy/better-github?path=src/App.tsx` with a locally signed dev JWT in `localStorage`. Confirmed the root README preview still rendered, and the file view rendered `.file-viewer-body` with 1741 `.line-number` cells starting at `1`, `2`, `3`, `4`, `5`, with no README panel on the file route.
